...
bitflags! {
    #[repr(C)]
    pub struct ELEMENT_STATE_BITS: u32 {
        /// Zero state.
        const STATE_NONE = 0x00000000;

        /// Element is a link.
        ///
        /// E.g. `<a href`.
        const STATE_LINK = 0x00000001;

        /// Mouse over the element at the moment.
        const STATE_HOVER = 0x00000002;
        /// Element is pressed.
        ///
        /// Commonly used by `<button>` or `<a>` elements.
        const STATE_ACTIVE = 0x00000004;
        /// Element is focused.
        const STATE_FOCUS = 0x00000008;

        /// Element was visited.
        ///
        /// For example, a link that was clicked.
        const STATE_VISITED = 0x00000010;
        /// Current (hot) item.
        const STATE_CURRENT = 0x00000020;
        /// Element is checked (or selected).
        const STATE_CHECKED = 0x00000040;
        /// Element is disabled.
        const STATE_DISABLED = 0x00000080;
        /// Readonly input element.
        const STATE_READONLY = 0x00000100;

        /// Expanded state - e.g. nodes in tree view.
        ///
        /// Mutually exclusive with `STATE_COLLAPSED`.
        const STATE_EXPANDED = 0x00000200;

        /// Collapsed state - e.g. nodes in tree view.
        ///
        /// Mutually exclusive with `STATE_EXPANDED`.
        const STATE_COLLAPSED = 0x00000400;

        /// One of fore/back images was requested but is not delivered.
        const STATE_INCOMPLETE = 0x00000800;
        /// Is animating currently.
        const STATE_ANIMATING = 0x00001000;
        /// Will accept focus.
        const STATE_FOCUSABLE = 0x00002000;

        /// Anchor in selection (used with current in selects).
        const STATE_ANCHOR = 0x00004000;
        /// This is a synthetic element - i.e. don't emit it's head/tail.
        const STATE_SYNTHETIC = 0x00008000;
        /// A popup element is shown for this particular element.
        const STATE_OWNS_POPUP = 0x00010000;

        /// Focus gained by tab traversal.
        const STATE_TABFOCUS = 0x00020000;
